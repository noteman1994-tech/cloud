...
const DEFAULT_SITE_NAME = 'cloud'
const DEFAULT_DOC_URL = 'https://www.feishu.cn/'

function normalizeSiteName(value?: string): string {
  const trimmed = value?.trim() || ''
  if (!trimmed || trimmed === 'Sub2API') return DEFAULT_SITE_NAME
  return trimmed
}

function normalizeDocUrl(value?: string): string {
  const trimmed = value?.trim() || ''
  return trimmed || DEFAULT_DOC_URL
}
...
export const useAppStore = defineStore('app', () => {
  ...
  function applySettings(config: PublicSettings): void {
    ...
    docUrl.value = normalizeDocUrl(config.doc_url)
